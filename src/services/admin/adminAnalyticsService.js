...
export const generateReportData = async (query) => {
    ...
    let paymentStats = { upi: 0, cod: 0, wallet: 0, card: 0 };
    let totalPaymentRevenue = metrics.totalSales || 1; // Prevent division by zero

    rawPaymentData.forEach(p => {
        const method = p._id ? p._id.toUpperCase() : '';
        if (method.includes('UPI')) paymentStats.upi += p.revenue;
    ...
    paymentStats.upiPercent = Math.round((paymentStats.upi / totalPaymentRevenue) * 100);
