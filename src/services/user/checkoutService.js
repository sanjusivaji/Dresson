...
export const processOrder = async (userId, selectedAddress, paymentMethod) => {
    const cart = await cartService.getUserCartData(userId);                                             // For 'cross checking' the data in 'cart' and 'recreate' it based on 'userId'.                    
    ...
    const rawCart = await checkoutRepository.findCartByUserIdDoc(userId);                               // It retrieve all 'cart' data include 'coupon'(but in 'cart' created above 'no' coupon data) data because we put 'appliedCoupon', 'discountAmount' like fields in cart model.
    ...
    const mappedItems = cart.items.map(item => {
        ...
        return { product: item.product, variantName: item.variantName, variantSku: item.sku, quantity: item.quantity, price: item.price, taxRate: item.taxRate || 0 };
    });    
    let discount = rawCart.discountAmount || 0;    
    let shipping = SHIPPING_CONFIG.STANDARD_FEE;
    if ((subTotal + totalTax - discount) >= SHIPPING_CONFIG.FREE_SHIPPING_THRESHOLD) discount += SHIPPING_CONFIG.STANDARD_FEE;      
    ...
    if (['razorpay', 'card'].includes(paymentMethod)) {                                                  // Here '[ ]'(ie 'array literals')used for create a new array and when 'user' select 'paymentMethod' is 'razorpay' and 'includes()'(ie the built-in 'js' method)return 'true' and then 'if condition' alos return 'true'.                             
       ...
       if (rawCart.appliedCoupon) {
            await recordCouponUsage(rawCart.appliedCoupon, userId);                                      // It used for add 'userId' and its 'count' into 'usedBy' array.                
        }                
