...
// For 'reduce' the 'stock' of exact variant from the product array and 'totalStock'
export const deductProductStock = async (productId, variantIdentifier, quantity) => {
    const qtyToDeduct = Number(quantity) || 1;
    const isObjectId = /^[0-9a-fA-F]{24}$/.test(variantIdentifier);
    const variantMatch = isObjectId 
        ? { _id: variantIdentifier } 
    ...
    return await Product.updateOne(
        ...
        { 
            $inc: { 
                "variants.$.stock": -qtyToDeduct, 
                "totalStock": -qtyToDeduct        
            } 
        }       
    );
};
